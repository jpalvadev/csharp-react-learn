...
type MultiSelectContextType = {
    ...
    //
    searchValue: string; // <-- Nuevo
    setSearchValue: (value: string) => void; // <-- Nuevo
};
...
export function MultiSelect({
    ...
    const [searchValue, setSearchValue] = useState(''); // <-- Nuevo estado

    function toggleValue(value: string) {
        const getNewSet = (prev: Set<string>) => {
            if (single) {
                return prev.has(value)
                    ? new Set<string>()
...
export function MultiSelectContent({
    ...
    const { setSearchValue } = useMultiSelectContext(); // <-- Consumimos el set
        ...
        <>
            ...
            <PopoverContent className="min-w-[var(--radix-popover-trigger-width)] p-0">
                <Command {...props}>
                    {canSearch ? (
                        <CommandInput
                            onValueChange={setSearchValue} // <-- CAPTURAMOS EL FILTRO AQUÍ
                            placeholder={
